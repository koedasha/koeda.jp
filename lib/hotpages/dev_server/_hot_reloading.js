...
function reloadCSS() {
  ...
  links.forEach(link => {
    ...
    const newHref = cacheBustedUrl(href);
    link.href = newHref;
  });
}

const hotReloadingWs = new WebSocket("ws://localhost:4000");

hotReloadingWs.onopen = function (event) {
  log("Hot reloading enabled.")
};

hotReloadingWs.onmessage = async function (event) {
  let data;
  ...
  switch (data.action) {
    case "reload:html":
      ...
      document.dispatchEvent(new CustomEvent("hotpages:hot-reload"));
      log("Hot reloaded HTML.")
      break;
    case "reload:css":
      reloadCSS();
      log("Hot reloaded CSS.")
    case "reload:js":
